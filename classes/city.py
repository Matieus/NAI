...
def create_dict_for_city(
    name: str, pm10: float, pm25: float, o3: float, no2: float, so2: float
) -> dict:
    """
    Parameters
    ----------
    ...
    """
    return {"name": name, "pm10": pm10, "pm25": pm25, "o3": o3, "no2": no2, "so2": so2}
...
class City:
    ...
    def _set_city_value(self, key: str, value: [float, str]):
        """
    ...
    def _get_city_data(self, id: int):
        ...
        """
        try:
            r = requests.get(f"{GIOS_ENDPOINTS['index']}/{id}")
            data = r.json()
    ...
    def _get_air_particles(self, id: int):
        ...
        """
        try:
            r = requests.get(f"{GIOS_ENDPOINTS['air_particles']}/{id}")
            data = r.json()
            ...
            self._set_city_value(key=key, value=latest_value)
        except Exception as e:
            print(f"Something went wrong: {str(e)}")
